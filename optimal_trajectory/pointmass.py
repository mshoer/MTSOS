...
muf = .3            # coefficient of friction between tires and ground
gravity = 9.81      # gravity


def define_params(mass, Wf, muf, gravity):
    """
    params derived from vehicle configuration
    """
...
def friction_circle(Fmax):
    """
    create boundary for friction circle
    """
...
def simulate(b, a, u, params, int_method ='rk6', plot_results=True):
    ...
    if int_method == 'odeint':
        print('using Runge Kutta 45 order integration')
        for j in range(num_wpts-1):
    ...
    if plot_results:
        ...
        plt.legend()
        np.savetxt("speed.csv", np.sqrt(vx**2+vy**2), delimiter=",", fmt='%s')
...
def optimize(path, params, plot_results=True):
    ...
    solution = problem.solve(solver=cv.ECOS_BB, verbose=False)
...
if __name__ == "__main__":
    ...
    # path can be 'qph', 'ellipse', 'wave', 'corner', rectangular2D', 'random', 'map0'
